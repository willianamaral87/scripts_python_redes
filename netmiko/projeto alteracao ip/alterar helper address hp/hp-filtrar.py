...
# Lista conteudo_todo arquivo de configuração do switch
lista = []
...
with open('config_hp/arquivo.txt', 'r') as tex:
    for linha in tex:
...
for i in range(0,len(lista)):
    ...
    re_inicio = re.search(r'interface Vlan-interface',lista[i])
    ...
    if len(sub_lista) > 0:      
        for j in range(0, len(sub_lista)):
            # r1 = re.search(r'relay server-address 10.15.80.1',sub_lista[j])
            # if r1 is not None:
            #     contem_80_1 = True
        
            r2 = re.search(r'relay server-address 10.15.80.2',sub_lista[j])
            if r2 is not None:
                contem_80_2 = True
...
#             if contem_80_1:
#                 lista_result.append('undo dhcp relay server-address 10.15.80.1')
#                 lista_result.append('dhcp relay server-address 10.93.80.100')
                
            if contem_80_2:
                lista_result.append('undo dhcp relay server-address 10.15.80.2')
                lista_result.append('dhcp relay server-address 10.92.82.2')

print(f'Foram encontradas {cont_interface} interfaces VLANs.')
            
# Salvar em disco o arquivo
with open('output_hp/saidaarquivo.txt', 'w') as texto:
